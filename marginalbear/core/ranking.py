...
def pos_idf_jaccard_similarity(query_vocabs, doc_vocabs, doc_num=500000.0):
    ...
    w_inter = [v.weight * v.quality * math.log(doc_num / max(1.0, v.titlefreq)) for k, v in inter.items()]
    w_union = [v.weight * v.quality * math.log(doc_num / max(1.0, v.titlefreq)) for k, v in union.items()]
