...
def pos_idf_jaccard_similarity(query_vocabs, doc_vocabs, doc_num=10000.0):
    ...
    w_inter = [v.weight * math.log(doc_num / min(1.0, v.postfreq)) for k, v in inter.items()]
    w_union = [v.weight * math.log(doc_num / min(1.0, v.postfreq)) for k, v in union.items()]
